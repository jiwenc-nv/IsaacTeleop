...
#. Install the example dependencies from a LeRobot source checkout. The LeRobot extras cover the
   ...
   .. tab-item:: XR controller
      ...
      .. note::
         ...
         **not** need to record anything. To tailor it to your setup, back-drive the arm to the
         ...
         per arm by ``--robot.id``, so a later run with the same ``--robot.id`` will pick it up
